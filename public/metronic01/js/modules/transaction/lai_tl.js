...
ClassicEditor.create(document.querySelector("#catatanLAI"))
  .then((newEditor) => {
    editor = newEditor;
  })
  .catch((error) => {
...
ClassicEditor.create(document.querySelector("#catatanLAIEdit"))
  .then((newEditor) => {
...
ClassicEditor.create(document.querySelector("#kesimpulanLAI"))
  .then((newEditor) => {
...
ClassicEditor.create(document.querySelector("#kesimpulanLAIEdit"))
  .then((newEditor) => {
...
$("#kt_datatable1 tbody").on("click", ".lihatCatatan", function () {
  ...
  button_action({
    ...
    btnLabel: "Ubah Data",
    type: "edit",
    url: "lai_tindak_lanjut/edittemuan",
    modal: "modalDeskripsi",
    form: "form_edit",
  });

  $.ajax({
    url: BASE + "lai_tindak_lanjut/getedit",
    data: { id: id },
    method: "post",
    success: function (data) {
      ...
      if (name == "Kesimpulan") {
        $("#viewDeskripsi").html(lai.data[0].KESIMPULAN);
      } else {
        $("#viewDeskripsi").html(lai.data[0].CATATAN);
      }

      // if (
      //   lai.data[0].FILE_TEMUAN_LTK != "" &&
      //   lai.data[0].FILE_TEMUAN_LTK != null
      // ) {
      //   $("#ltkAwalLink").attr("href", BASE + lai.data[0].FILE_TEMUAN_LTK);
      //   $("#ltkAwal").show();
      // } else {
      //   $("#ltkAwal").hide();
      // }
      ...
      // if (
      //   lai.data[0].FILE_TL_LTK != "" &&
      //   lai.data[0].FILE_TL_LTK != null
      // ) {
      //   $("#ltkProgressLink").attr("href", BASE + lai.data[0].FILE_TL_LTK);
      //   $("#ltkProgress").show();
      // } else {
      //   $("#ltkProgress").hide();
      // }

      // var tl = lai.data[0].DESKRIPSI_TL;
      // if (tl === "" || tl === null) {
      //   document.getElementById("viewTL").classList.add("text-danger");
      //   if (
      //     lai.data[0].FILE_TEMUAN_LTK != "" &&
      //     lai.data[0].FILE_TEMUAN_LTK != null &&
      //     (lai.data[0].FILE_TEMUAN_TL == "" ||
      //       lai.data[0].FILE_TL_LTK == null)
      //   ) {
      ...
      //   $("#viewTL").html(lai.data[0].DESKRIPSI_TL);
      // }
    },
  });
});
...
$("#kt_datatable1 tbody").on("click", ".tombolEdit", function () {
  ...
  button_action({
    label: "Ubah Data Laporan Audit Internal",
    btnLabel: "Ubah Data",
    type: "edit",
    url: "lai_tindak_lanjut/edit",
    modal: "modalEdit",
    form: "form_edit",
  });
  ...
  $.ajax({
    ...
    success: function (data) {
      var lai = JSON.parse(data);
      console.log(lai);
      $("#idEdit").val(lai.data[0].ID_LAI);
      $("#idJadwalEdit").val(lai.data[0].ID_JADWAL);
      $("#idSeksiEdit").val(lai.data[0].ID_SEKSI);
      $("#noLaiEdit").val(lai.data[0].NO_LAI);
      $("#judulLaiEdit").val(lai.data[0].JUDUL_LAI);
      $("#emailPICEdit").val(lai.data[0].EMAIL_PIC);

      editorEdit.setData("");
      if (lai.data[0].CATATAN !== "" && lai.data[0].CATATAN !== null) {
        editorEdit.setData(lai.data[0].CATATAN);
      }

      kesimpulanEdit.setData("");
      if (lai.data[0].KESIMPULAN !== "" && lai.data[0].KESIMPULAN !== null) {
        kesimpulanEdit.setData(lai.data[0].KESIMPULAN);
      }

      $("#unggahanFileEdit").attr("href", BASE + lai.data[0].FILE_LAI);
      if (lai.data[0].FILE_LAI !== "" && lai.data[0].FILE_LAI !== null) {
        $("#igpFileEdit").show();
        $("#checkFileEdit").show();
        $("#isFileEdit").val("1");
      } else {
...
$("#idJadwal").on("change", function () {
  ...
  var idSeksi = selected.data("seksi");
  $("#idSeksi").val(idSeksi);
  console.log("idSeksi: " + idSeksi);
  ...
  var _handleTambah = function () {
    ...
    if (isSelectorExist("#form_tambah")) {
      ...
      $("#submit").on("click", function (e) {
        ...
        _input.append("catatanLAI", editor.getData());
        _input.append("kesimpulanLAI", kesimpulan.getData());
  ...
  var _handleEdit = function () {
    ...
    if (isSelectorExist("#form_edit")) {
      ...
      $("#submitEdit").on("click", function (e) {
        ...
        _input.append("catatanLAI", editorEdit.getData());
        _input.append("kesimpulanLAI", kesimpulanEdit.getData());
    ...
    if (isSelectorExist("#form_progress")) {
      ...
      $("#submitProgress").on("click", function (e) {
        ...
        validation.validate().then(function (status) {
          ...
          if (status == "Valid") {
            $.ajax({
              url: "lai_tindak_lanjut/progress",
              method: "POST",
              data: _input,
              contentType: false,
              cache: false,
...
$(document).on("click", ".tombolPrint", function () {
  ...
  $.ajax({
    url: BASE + "lai_tindak_lanjut/print",
    data: { id: id },
    method: "post",
    success: function (data) {
...
function getPDF2() {
  ...
  // pdf.html(document.getElementById("PrintLAIPDF"), {
  //   callback: function () {
  //     //pdf.save('test.pdf');
  ...
  pdf.addHTML($("#PrintLAIPDF")[0], function () {
    pdf.save("HTML-Document.pdf");
  });
}
