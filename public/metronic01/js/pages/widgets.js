...
var KTWidgets = function () {
    ...
    var _initDaterangepicker = function () {
        ...
        var start = moment().year('2021').month('05').date('01');
        var end = moment();

        function cb(start, end, label = 'All Time') {
            var id_batch = $('#batchValue').val();
            // console.log(id_batch);
            var title = '';
            var range = '';

            if (label == 'All Time' || label == '') {
                title = 'All Time';
                range = '';
            ...
            $.ajax({
                ...
                success: function (response) {
                    if (isJson(response)) {
                        ...
                        if (data.status == true) {
                            totalLahan = data.data[0].TARGET_LUAS_TANAH;
                            $('#total_lahan').html(convert_number(data.data[0].TARGET_LUAS_TANAH));
                            $('.batchText').html(data.data[0].NM_BATCH);
                            $('#batchValue').val(data.data[0].ID_BATCH);
                        }
                    }
                }
                ...
                $.ajax({
                    url: BASE + 'dashboard/update_data',
                    data: {
                        start: dateStart,
                        end: dateEnd,
                        id_batch: id_batch
                    ...
                    success: function (response) {
                        if (isJson(response)) {
                            ...
                            // console.log(data)
                            if (data.status == true) {
                                $('#total_lahan').html(convert_number(totalLahan))

                                // tanah lahan
                                $('#pemilik_lahan').html(convert_number(data.data[0].TOTAL_TANAH))
                                $('#lahan_terukur').html(convert_number(data.data[0].TOTAL_LUAS_TANAH))
                                $('#persen_total_lahan').html(convert_persen(totalLahan, data.data[0].TOTAL_LUAS_TANAH) + ' %');

                                // berita 
                                $('#total_berita').html(convert_number(data.data[0].TOTAL_BERITA))
                                $('#lahan_berita').html(convert_number(data.data[0].TOTAL_TANAH_BERITA))
                                $('#persen_berita').html(convert_persen(totalLahan, data.data[0].TOTAL_TANAH_BERITA) + ' %');
                                $('#berita_bar').css('width', convert_persen(totalLahan, data.data[0].TOTAL_TANAH_BERITA) + '%');

                                // pemotongan 
                                $('#total_potong').html(convert_number(data.data[0].TOTAL_POTONG))
                                $('#lahan_potong').html(convert_number(data.data[0].TOTAL_TANAH_POTONG))
                                $('#persen_potong').html(convert_persen(totalLahan, data.data[0].TOTAL_TANAH_POTONG) + ' %');
                                $('#potong_bar').css('width', convert_persen(totalLahan, data.data[0].TOTAL_TANAH_POTONG) + '%');

                                // pks 
                                $('#total_pks').html(convert_number(data.data[0].TOTAL_PKS))
                                $('#lahan_pks').html(convert_number(data.data[0].TOTAL_TANAH_PKS))
                                $('#persen_pks').html(convert_persen(totalLahan, data.data[0].TOTAL_TANAH_PKS) + ' %');
                                $('#pks_bar').css('width', convert_persen(totalLahan, data.data[0].TOTAL_TANAH_PKS) + '%');

                                // pembayaran 
                                $('#total_bayar').html(convert_number(data.data[0].TOTAL_POTONG))
                                $('#lahan_bayar').html(convert_number(data.data[0].TOTAL_TANAH_POTONG))
                                $('#persen_bayar').html(convert_persen(totalLahan, data.data[0].TOTAL_TANAH_POTONG) + ' %');
                                $('#bayar_bar').css('width', convert_persen(totalLahan, data.data[0].TOTAL_TANAH_POTONG) + '%');
                            }
                        }
                    }
                })
        ...
        picker.daterangepicker({
            ...
            ranges: {
                'All Time': [moment().year('2021').month('06').date('01'), moment()],
                'Today': [moment(), moment()],
